...
void one_thread_run(std::string path,int thread_id){
    ...
    for (int j = 0; j < vin_name.size(); ++j) {
        ...
        fill_tensor_device_const(*d_tensor_in_p, 1.f);
    }
...
#define LOG_OUTPUT
        ...
        double mean_val = 0.0;
        bool flag_pass = true;
        float real_value=0.5280559f;
        for (int j = 0; j < tensor_out->valid_size(); j++) {
            if (fabs(ptr_out[j] - real_value) > 1e-5f) {
                flag_pass = false;
            }
            mean_val += ptr_out[j];
        ...
        if (!flag_pass) {
            LOG(FATAL) << "error in loop " << i <<","<<ptr_out[0]<<" != "<<real_value;
        }
#endif
    }
    my_time.end(ctx);
...
//        }
//
//        Worker<NV, AK_FLOAT, Precision::FP32>  workers(*iter, 2);
