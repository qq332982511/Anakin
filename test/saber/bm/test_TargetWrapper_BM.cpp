...
int main() {
    //bmdnn_init(&handle);
    typedef TargetWrapper<BM,__device_target> API;
    //int dev_count = 0;
    //API::get_device_count(dev_count);
    //std::cout << "dev_count: " << dev_count << std::endl;
    
    //bm_device_mem_t *pmem = new bm_device_mem_t();
    typedef typename PtrTrait<BM>::PtrType Ptype;

    Ptype pmem;
    std::cout << "mem addr before mem_alloc: " <<std::endl;//<< pmem << std::endl;
    API::mem_alloc(&pmem, 3*200*400);
    std::cout << "mem addr after  mem_alloc: " <<std::endl;//<< pmem << std::endl;
